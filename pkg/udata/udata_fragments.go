...
{{- end}}
  ...
  - name: "calico.service"
    ...
    content: |
     ...
     KillMode=mixed
     EnvironmentFile=/etc/kato.env
     Environment=CNI_URL=https://github.com/projectcalico/calico-cni/releases/download/v1.8.0
     Environment=CALICOCTL_URL=https://github.com/projectcalico/calico-containers/releases/download/v1.1.3
     Environment=CNI_PLUGINS=/var/lib/cni-plugins
     Environment=IMG=quay.io/calico/node:v1.1.3
...
{{- end}}
  ...
  - name: "marathon.service"
    ...
    content: |
     ...
     LimitNOFILE=8192
     EnvironmentFile=/etc/kato.env
     Environment=IMG=quay.io/kato/marathon:v1.4.3-1
     ExecStartPre=/opt/bin/zk-alive ${KATO_QUORUM_COUNT}
     ExecStartPre=/usr/bin/rkt fetch ${IMG}
     ExecStart=/usr/bin/rkt run \
      --net=host \
