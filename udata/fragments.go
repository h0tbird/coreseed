...
{{- end}}
  ...
  - name: "calico.service"
    ...
    content: |
     ...
     Environment=CALICOCTL_URL=https://github.com/projectcalico/calico-containers/releases/download/v1.0.0
     Environment=CNI_PLUGINS=/var/lib/cni-plugins
     Environment=IMG=quay.io/calico/node:v1.0.0
...
{{- end}}
  ...
  - name: "marathon.service"
    ...
    content: |
     ...
     LimitNOFILE=8192
     EnvironmentFile=/etc/kato.env
     Environment=IMG=quay.io/kato/marathon:v1.4.0-RC7
     ExecStartPre=/opt/bin/zk-alive ${KATO_QUORUM_COUNT}
     ExecStartPre=/usr/bin/rkt fetch ${IMG}
     ExecStart=/usr/bin/rkt run \
      --net=host \
  ...
  - name: "rexray.service"
    ...
    content: |
     ...
     Environment=REXRAY_URL=https://emccode.bintray.com/rexray/stable/0.6.3/rexray-Linux-x86_64-0.6.3.tar.gz
...
{{- end}}
  ...
  - name: "marathon-lb.service"
    ...
    content: |
     ...
     TimeoutStartSec=0
     KillMode=mixed
     Environment=IMG=mesosphere/marathon-lb:v1.5.0
     ExecStartPre=/usr/bin/rkt fetch --insecure-options=image docker://${IMG}
